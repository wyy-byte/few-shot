...
def build_patch_memory():
    """只用单张正常图当 PatchCore 模板，不再跑训练集"""
    mem_file = os.path.join(NPY_DIR, "once_patch_memory.npy")
    if os.path.exists(mem_file):
        return np.load(mem_file)

    print("【模板】用单张正常图生成 PatchCore 模板 ...")
    ...
    # 单模板记忆库：一条 NUM_CLASSES×feat_dim 向量
    mem = comp_vec.reshape(1, -1)           # (1, NUM_CLASSES*feat_dim)
    np.save(os.path.join(NPY_DIR, "once_patch_memory.npy"), mem)
    return mem

COMP_MEMORY = build_patch_memory()   # (1, NUM_CLASSES*feat_dim)
...
# ---------- 4. 主入口（只跑异常图） ----------
def main():
    print("=== PSAD 单模板 PatchCore 仅输出分数 ===")
