...
RESIZE = 512
BATCH = 2
REPRI_EPOCHS = 50       # 阶段-1：RePRI
FINE_EPOCHS  = 50       # 阶段-2：微调
LR_REPRI = 1e-3
LR_FINE  = 5e-5
WD = 1e-4
LAMBDA_CE  = 1.0
...
def get_five_names():
    names = sorted([os.path.splitext(os.path.basename(p))[0]
                    for p in glob.glob(f"{IMG_DIR}/*.png")])[:3]
    assert len(names) == 3, f"需要 3张图，实际 {len(names)}"
    return names
...
class PSADBackbone(nn.Module):
    ...
    def forward(self, x):
        ...
        x3 = self.layer3(x2)
        h, w = x.shape[2:]
        f1 = F.interpolate(x1, (h, w), mode="bilinear", align_corners=False)
        f2 = F.interpolate(x2, (h, w), mode="bilinear", align_corners=False)
        f3 = F.interpolate(x3, (h, w), mode="bilinear", align_corners=False)
...
def main():
    ...
    parser.add_argument("--stage", choices=["repri", "fine", "both"], default="both",
                        help="repri:仅预训练; fine:仅微调; both:连续执行")
